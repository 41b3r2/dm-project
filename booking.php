...
$sql = "SELECT Locationid, LocationSpot FROM package";
$resultBook = $conn->query($sql);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $required_fields = ['Fullname', 'Email', 'datetime', 'Destination'];
    $errors = [];
    ...
    if (empty($errors)) {
        ...
        $Email = $_POST['Email'];
        $Date = $_POST['datetime'];
        $LocationId = $_POST['Destination']; // This is now the LocationId
        $SpecialRequest = $_POST['SpecialRequest'];
        $schedule = $_POST['schedule'];
        
        $Userid = $_SESSION['Userid'];  // Get logged-in user ID
        
        // Fetch the LocationSpot based on the selected LocationId
        $location_query = "SELECT LocationSpot FROM package WHERE Locationid = ?";
        ...
        if ($result->num_rows > 0) {
            ...
            $Destination = $row['LocationSpot'];
            
            $sql = "INSERT INTO booking (schedule) VALUES ('$schedule')";
            $sql = "INSERT INTO booking (Userid, Locationid, Fullname, Email, Date, schedule, Destination, SpecialRequest)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
            
            // Prepare statement
            if ($stmt = $conn->prepare($sql)) {
                // Bind parameters
                $stmt->bind_param('iissssss', $Userid, $LocationId, $Fullname, $Email, $Date, $schedule, $Destination, $SpecialRequest);
                
                // Execute the query
                if ($stmt->execute()) {
                    echo "<div class='alert alert-success'>Booking successfully made!</div>";
                } else {
            ...
            echo "<div class='alert alert-danger'>Error: Invalid location selected.</div>";
        }
    } else {
        // Display errors
...
<!-- The HTML form remains unchanged -->
...
<script>
    // Function to format current date and time with seconds
    ...
    // Initial set and start interval
    updateDateTime();
                            ...
                            <div class="col-md-6">
                                <div class="form-floating">
                                    ...
                                    >
                                        ...
                                        $package_query = "SELECT Locationid, LocationSpot FROM package";
